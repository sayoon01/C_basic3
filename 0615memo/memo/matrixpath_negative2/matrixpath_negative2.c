...
int matrixPath_memo(int** m, int r, int c, int i, int j, int** M, int** M1)
{
	...
	if (i == 0 && j == 0) {
		...
		M[i][j] = matrixPath_memo(m, r, c, i, j - 1, M, M1) + m[i][j];
		return M[i][j];
		...
		M[i][j] = matrixPath_memo(m, r, c, i - 1, j, M, M1) + m[i][j];
		return M[i][j];
	...
	{
		int a, b;
		a = matrixPath_memo(m, r, c, i - 1, j, M, M1);
		b = matrixPath_memo(m, r, c, i, j - 1, M, M1);

		M[i][j] = ((a < b) ? a : b) + m[i][j];
		return M[i][j];

		/*M[i - 1][j] = matrixPath_memo(m, r, c, i = 1, j, M, M1) + m[i][j];
		M[i][j - 1] = matrixPath_memo(m, r, c, i, j - 1, M, M1) + m[i][j];

		return ((M[i - 1][j] < M[i][j = 1]) ? M[i - 1][j] : M[i][j - 1]);*/
